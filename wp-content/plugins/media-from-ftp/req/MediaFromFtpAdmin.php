...
COMMANDLINESET2;
	...
	function search_register_page() {
		...
		foreach ( $files as $file ){
			...
			if ( $new_file ) {
				$searchfiles[$pagecount] = $file;
				$search_ext[$pagecount] = $ext;
				$search_new_url[$pagecount] = $new_url;
				++$pagecount;
			}
		}
		unset($files);
		$pageallcount = $pagecount;
		...
		if ( $pageallcount > 0 ) {
			...
			if ($mediafromftp_settings['search_display_metadata']){
				echo __('Select').' & '.__('Thumbnail').' & '.__('Metadata');
			} else {
				echo __('Select').' & URL';
			}
			?>
		...
		$postcount = 0;
		for ( $i = $pagebegin-1; $i <= $pageend-1; $i++ ) {
			if ( $i < $pageallcount ) {
				$file = $searchfiles[$i];
				$ext = $search_ext[$i];
				$new_url = $search_new_url[$i];
				echo $mediafromftp->input_html($ext, $file, $new_url, $postcount, $mediafromftp_settings);
				++$postcount;
			}
		}
